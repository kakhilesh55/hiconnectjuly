

  <section id="about-us" class="mt-5">
<br><br>
  <div class="container-fluid">
    <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
      <div class="row">
    <!-- <div class="about-box text-center">
          <div class="about-red d-flex align-items-center">
            <h3>Your first step</h3>
          </div>
          <div class="about-black ms-3 d-flex align-items-center justify-content-center">
            <div>
            <h3>to success</h3>
            <p>about us</p>
          </div>
          </div>
        </div> -->
		<div class="pricing-header-wrapper text-center">
	<h1>About HiConnect  </h1>
<p>"Our mission is to help businesses, professionals, and people develop deeper, longer-lasting relationships by providing real-time information that nurtures networking and personal relationships."</p>	
</div>
      </div>
    </div>
  </div>
 
    <div class="container-fluid pt-5 mt-5 pb-5">
      <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
        <div class="row d-flex align-items-center">
          <div class="col-lg-6 order-2 order-lg-1 mt-3 mt-lg-0 aos-init aos-animate hi-abt-content" data-aos="fade-up" data-aos-delay="100">
            <h2>Who We Are ?</h2>
            <p>We're a group of adventurous individuals who value healthy relations. So we research and develop the solutions to make deep and sustainable networking.</p>
              <p>We will continue to innovate and expand developing new solutions and designs using our cutting-edge technology and integrations to reshape how the business world connects. </p> 
			 
          </div>
          <div class="col-lg-6 order-1 order-lg-2 text-center aos-init aos-animate" data-aos="fade-up" data-aos-delay="200">
            <img src="<?php echo base_url(); ?>css1/images/black-mobile.png" alt="">
          </div>
        </div>
      </div>
    </div>


    <div class="container-fluid pt-5 pb-5 mt-5">
      <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
        <div class="row hi-tap text-center">
          <h4>Most businesses and people change one or<br>
            more elements of their business cards every year for better networking.</h4>
            <h3>A Gamechanger for Businesses in the Digital Economy.</h3>
        </div>
      </div>
    </div>

    <div class="container-fluid pt-5 mt-5 pb-5">
      <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
        <div class="row d-flex align-items-center">
          <div class="col-lg-6 order-2 order-lg-1 mt-3 mt-lg-0 aos-init aos-animate  hi-abt-content" data-aos="fade-up" data-aos-delay="100">
            <h2>Hi Connect </h2>
            <p>Hi Connect is a networking platform that helps professionals and businesses worldwide to modernise networking. </p>
<p>Unlike other solutions on the market, Hi Connect is not just a business card to share your info, we encourage more in-person interactions to form authentic connections with those around us.  </p>
          </div>
          <div class="col-lg-6 order-1 order-lg-2 text-center aos-init aos-animate" data-aos="fade-up" data-aos-delay="200">
            <img src="<?php echo base_url(); ?>css1/images/black-mobile.png" alt="">
          </div>
        </div>
      </div>
    </div>



    <div class="container-fluid pb-5">
      <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
        <div class="row lets-simple text-center">
          <h2>Let's make it simple for you.</h2>

          <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
            <div class="card text-center border-0">
              <div class="card-body">
                <img class="card-img-top pb-4 pt-4" src="<?php echo base_url(); ?>css1/images/lets01.png" alt="How it image">
                <h4 class="card-title">One tap, infinite possibilities</h4>
                <p class="card-text"></p>
              </div>
            </div>
          </div>
          <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
            <div class="card text-center border-0">
              <div class="card-body">
                <img class="card-img-top pb-4 pt-4" src="<?php echo base_url(); ?>css1/images/lets02.png" alt="How it image">
                <h4 class="card-title">Save Money and Trees</h4>
             <!--   <p class="card-text">Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequatur, aliquam.</p> -->
              </div>
            </div>
          </div>
          <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
            <div class="card text-center border-0">
              <div class="card-body">
                <img class="card-img-top pb-4 pt-4" src="<?php echo base_url(); ?>css1/images/lets03.png" alt="How it image">
                <h4 class="card-title">Stand out with Hi Connect</h4>
                
              </div>
            </div>
          </div>

        </div>
      </div>
    </div>
    <div class="container-fluid">
      <div class="container col-xl-9" data-aos="fade-up" data-aos-delay="100">
        <div class="row abt-quote text-center">
          <h3>“Make an impression when networking with the Hi Connect digital business card. Share your info with a single tap on an iPhone or Android device. There is no app required.”</h3>
        </div>
      </div>
    </div>
  </section>
